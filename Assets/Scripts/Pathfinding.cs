...
public class Pathfinding : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (Instance != null)
        {
            Debug.LogError("There should never be two Pathfinding." + transform + " - " + Instance);
            Destroy(gameObject);
            return;
        }
        Instance = this;
    ...
    public void SetUp(int width, int height, float cellSize)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int z = 0; z < height; z++)
            {
                ...
                if (Physics.Raycast(worldPostion + Vector3.down * raycastOffsetDistance, Vector3.up, raycastOffsetDistance * 2, obstaclesLayerMask))
                {
